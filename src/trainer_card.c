...
struct TrainerCardData
{
    ...
    u8 textNuzlockeRevives[70]; //tx_randomizer_and_challenges
...
static bool8 ShouldShowNuzlockeRevives(void); //tx_randomizer_and_challenges
static void BufferNuzlockeRevives(void); //tx_randomizer_and_challenges
static void PrintNuzlockeRevivesOnCard(void); //tx_randomizer_and_challenges
...
//      PrintBattleFacilityStringOnCard();
        PrintNuzlockeRevivesOnCard(); //tx_randomizer_and_challenges
        break;
    case 7:
        PrintStickersOnCard();
        break;
...
static void BufferTextsVarsForCardPage2(void)
{
    ...
    BufferNuzlockeRevives(); //tx_randomizer_and_challenges
}
...
// The revive count is stored in the save block rather than in struct TrainerCard,
// so it is only meaningful on the player's own card, never on a linked one.
static bool8 ShouldShowNuzlockeRevives(void)
{
    if (sData->isLink)
        return FALSE;

    // Both the full Nuzlocke and the mini ("easy") mode send fainted Pokemon to
    // the cemetery, so both can spend revives. Keyed off the settings rather than
    // IsNuzlockeActive() so the count stays visible once the player is Champion.
    return gSaveBlock1Ptr->tx_Challenges_Nuzlocke || gSaveBlock1Ptr->tx_Nuzlocke_EasyMode;
}

static void BufferNuzlockeRevives(void)
{
    if (ShouldShowNuzlockeRevives())
        ConvertIntToDecimalStringN(sData->textNuzlockeRevives, GetNuzlockeRevivesLeft(), STR_CONV_MODE_LEFT_ALIGN, 2);
}

// Row 5 is free because PrintBattleFacilityStringOnCard, its only other user, is
// disabled in PrintAllOnCardBack. It is also the last row window 1 can fit.
static void PrintNuzlockeRevivesOnCard(void)
{
    if (ShouldShowNuzlockeRevives())
        PrintStatOnBackOfCard(5, gText_NuzlockeRevives, sData->textNuzlockeRevives, sTrainerCardStatColors);
}
