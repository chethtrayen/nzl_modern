...
// A fainted nuzlocke mon is locked in the PC until the player is Champion.
// While cemetery withdrawal is enabled it can be freed early, but only by
// spending one of the player's remaining revives. //tx_randomizer_and_challenges
static bool8 IsMonNuzlockeLocked(bool8 hasNuzlockeRibbon)
{
    if (!hasNuzlockeRibbon)
        return FALSE;

    if (FlagGet(FLAG_IS_CHAMPION))
        return FALSE;

    if (TX_NUZLOCKE_CEMETERY_WITHDRAW && GetNuzlockeRevivesLeft() != 0)
        return FALSE;
    ...
    case 2:
        ...
        case MENU_PLACE:
            if (sIsMonBeingMoved && sCursorArea == CURSOR_AREA_IN_PARTY
                && IsMonNuzlockeLocked(GetMonData(&sStorage->movingMon, MON_DATA_NUZLOCKE_RIBBON))) //tx_randomizer_and_challenges
            {
                sStorage->state = 7;
                break;
            }
        ...
        case MENU_TAKE:
            ...
            break;
        case MENU_GIVE:
            if (IsMonNuzlockeLocked(GetCurrentBoxMonData(sCursorPosition, MON_DATA_NUZLOCKE_RIBBON))) //tx_randomizer_and_challenges
            {
                sStorage->state = 7;
                break;
            }
...
static void Task_WithdrawMon(u8 taskId)
{
    ...
    case 0:
        ...
        else if (IsMonNuzlockeLocked(GetCurrentBoxMonData(sCursorPosition, MON_DATA_NUZLOCKE_RIBBON))) //tx_randomizer_and_challenges
        {
            if ((gSaveBlock1Ptr->tx_Nuzlocke_EasyMode) && (!IsNuzlockeActive()))
                PrintMessage(MSG_NUZLOCKE);
            else
        ...
        else if (sIsMonBeingMoved && IsMonNuzlockeLocked(GetMonData(&sStorage->movingMon, MON_DATA_NUZLOCKE_RIBBON))) //tx_randomizer_and_challenges
        {
            if ((gSaveBlock1Ptr->tx_Nuzlocke_EasyMode) && (!IsNuzlockeActive()))
                PrintMessage(MSG_NUZLOCKE);
            else
...
static void SetPlacedMonData(u8 boxId, u8 position)
{
    ...
    if (boxId == TOTAL_BOXES_COUNT)
    {
        ...
        // Spend a revive only when a fainted mon is actually coming out of a box.
        // Re-placing one inside the party must not consume a charge. //tx_randomizer_and_challenges
        if (TX_NUZLOCKE_CEMETERY_WITHDRAW
            && sMovingMonOrigBoxId != TOTAL_BOXES_COUNT
            && GetMonData(&sStorage->movingMon, MON_DATA_NUZLOCKE_RIBBON)
            && !FlagGet(FLAG_IS_CHAMPION))
        {
            SpendNuzlockeRevive();

            // Leaving the cemetery clears the fainted mark.
            value = FALSE;
            SetMonData(&sStorage->movingMon, MON_DATA_NUZLOCKE_RIBBON, &value);
        }
