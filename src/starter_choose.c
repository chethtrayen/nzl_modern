...
u16 GetStarterPokemon(u16 chosenStarterId)
{
    ...
    // Clamp before indexing. The array was previously read with the caller's raw id,
    // and the guard used > rather than >=, so an id of STARTER_MON_COUNT both read
    // sStarterMon out of bounds and then went unclamped into the lookups below.
