...
// Revives are stored as a spent count, so a save from before the field existed
// reads 0 and yields the full allowance rather than none. //tx_randomizer_and_challenges
u8 GetNuzlockeRevivesLeft(void)
{
    if (gSaveBlock1Ptr->tx_Nuzlocke_RevivesUsed >= TX_NUZLOCKE_REVIVES)
        return 0;

    return TX_NUZLOCKE_REVIVES - gSaveBlock1Ptr->tx_Nuzlocke_RevivesUsed;
}

void SpendNuzlockeRevive(void)
{
    // 4-bit field; never wrap back around to a fresh allowance.
    if (gSaveBlock1Ptr->tx_Nuzlocke_RevivesUsed < 15)
        gSaveBlock1Ptr->tx_Nuzlocke_RevivesUsed++;
}
...
static const u8 sLevelCapTable_Normal[] = 
{
    ...
    [LEVEL_CAP_BADGE_2]     = 27,
    [LEVEL_CAP_BADGE_3]     = 32,
    [LEVEL_CAP_BADGE_4]     = 35,
    [LEVEL_CAP_BADGE_5]     = 41,
    [LEVEL_CAP_BADGE_6]     = 47,
    [LEVEL_CAP_BADGE_7]     = 54,
    [LEVEL_CAP_BADGE_8]     = 63,
};
